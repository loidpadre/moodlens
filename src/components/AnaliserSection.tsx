...
export default function AnaliserSection() {
    ...
    const [estimatedTime, setEstimatedTime] = useState(null);  // Para armazenar o tempo estimado
    const KEY_TOKEN = process.env.NEXT_PUBLIC_HF_API_KEY
    const URL = process.env.NEXT_PUBLIC_URL
    const analizer = async (e) => {

        e.preventDefault();
        setLoading(true);
        setError(null);  // Limpar qualquer erro anterior
        setEstimatedTime(null);  // Limpar tempo estimado anterior

        try {
            if(text === "" || null){
                setLoading(false)
                return alert("Preacha o campo!")
            }
